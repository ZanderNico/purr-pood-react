...
const addCarts = async (cartData: CreateCartsData) => {
  try {
    ...
    console.log("SA SERVICE ADDCART", response.data)
...
const getUsersWithCartsAndPetFood = async () => {
  try {
    const token = getTokenAuth();
    const response = await axios.get(`${base_url}/carts/users-carts`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    });
    console.log("SA SERVICE GETUSERCART", response.data.data)
...
const getCartWithUserById = async (userId: number) => {
  try {
    const token = getTokenAuth();
    const response = await axios.get(
      `${base_url}/carts/users-carts/${userId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    console.log("SA SERVICE", response.data)
    return response.data
  } catch (error) {
    console.error(error);
    throw error;
